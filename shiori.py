...
class LibraryManager:
    ...
    def scan_directory(self, directory):
        ...
        if len(subdirs) == 0:
            ...
            if self.is_songdir(files):
                ...
                if (len(txtfiles) > 1):
                    logging.info(
                        "Found 2 or more text files in likely song directory " + directory)
                    for txtfile in txtfiles:
                        self.parse_songfile(
                            join(directory, txtfile), directory)
                elif (len(txtfiles) == 0):
                    logging.warning(
                        "Could not find text file in likely song directory " + directory)
                else:
                    logging.info(
                        "Found likely ultrastar file in directory " + directory)
                    self.parse_songfile(
                        join(directory, txtfiles[0]), directory)
            else:
    ...
    def is_songdir(self, filenames):
        ...
        if (not (".mp3".upper() in extensions or ".ogg".upper() in extensions or ".aac".upper() in extensions) and
            ...
            logging.error(
                "No sound or video file found for the following directory!")
            return False
    ...
    def parse_songfile(self, filename, dir):
        ...
        try:
            ...
            try:
                ...
                song_data = ultraparse.SongFile(
                    iter(f.readlines()), filename, dir)
                if song_data.parse():
                    self.library.append(song_data)
                f.close()
            except UnicodeDecodeError as e:
                logging.error(
                    "well balls, apparently the encoding is wonky in file " + filename + "; " + str(e))
        except Exception as e:
            logging.error(
                "Encountered unexpected error parsing file " + filename + ":\n")
            logging.exception(e)


def run_scan():
    parser = argparse.ArgumentParser(
        description="Scan directory for ultrastar songs, then export them in a given format")
